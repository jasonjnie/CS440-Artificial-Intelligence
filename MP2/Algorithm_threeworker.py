...
DEF_WEIGHT = [0.2,0.8]
EAT_WEIGHT = 1
AB_DEPTH = 4 # 5 for basic implementation & 5*10 board bonus, 5 for threebase bonus
MINI_DEPTH = 3
OFFENSIVE = 1
DENFENSIVE = 2
node = 0;
			...
			for x in range(self.Board.width):
				if board[y][x] == player:
					if player == YOU:
						if y != 0:
							if (y != 0) and (x==0):
								if (board[y-1][x]==EMPTY) and (board[y-1][x+1]==EMPTY):
									score += 5*(len(board)-y)*(len(board)-y);
								else:
								...
								if (board[y-1][x-1]==EMPTY) and (board[y-1][x]==EMPTY) and (board[y-1][x+1]==EMPTY):
									score += 5*(len(board)-y)*(len(board)-y);
								else:
									score += (len(board)-y)*(len(board)-y);
