...
class Board:
    ...
    def next_move(self,Board,Player,Piece,Move):
        new_board = deepcopy(self.board)        ################################################# Deepcopy?
        if Player==BLACK:                                      # Black, Moving Downwards
            if Move==LEFT:                                     # Left-Diagonal
                self.board[Piece[0]+1][Piece[1]+1][2] = BLACK  ############################################# Capture?
    ...
    #def next_position(self,Prev_Posiiton, Move):
