...
X = matfile('X_train_ternary.mat');
X = X.X;
y = matfile('y_train_ternary.mat');
y = y.y;
...
%         y_data = y(index',:);%y_data: epoch * 1
        ...
        y_ = [ones(size(X_data,1), 1) X_data]*initial_Theta';%epoch * 785
        [y_ index] = max(y_');%epoch * 1(index: predicted labels)
        ...
        for i=1:size(X_data,1)%epoch
            ...
            %y_data(i) - true label
            initial_Theta(index(i),:)=initial_Theta(index(i),:)-learning_rate*[ones(1, 1) X_data(i,:)];%1x785
            initial_Theta(y_data(i),:)=initial_Theta(y_data(i),:)+learning_rate*[ones(1, 1) X_data(i,:)];%1x785
...
%% testing:
X = matfile('X_test_ternary.mat');
X_test = X.X;
y = matfile('y_test_ternary.mat');
y_test = y.y;
 

m = size(X_test, 1);
a = [ones(m, 1) X_test]* initial_Theta';%epoch x 10
[data,index] = max(a');
y_test = y_test+1;
